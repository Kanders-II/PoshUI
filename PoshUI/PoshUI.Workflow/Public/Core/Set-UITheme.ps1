    ...
    process {
        try {
            ...
            if ($hasDualMode) {
                ...
                $script:CurrentWorkflow.Branding['ThemeOverrides'] = $baseOverrides
                $script:CurrentWorkflow.Branding['ThemeOverridesLight'] = $mergedLight
                $script:CurrentWorkflow.Branding['ThemeOverridesDark'] = $mergedDark
                Write-Verbose "Dual-mode theme configured: Base=$($baseOverrides.Count), Light=$($mergedLight.Count), Dark=$($mergedDark.Count)"
            }
            else {
                $script:CurrentWorkflow.Branding['ThemeOverrides'] = $baseOverrides
                $script:CurrentWorkflow.Branding.Remove('ThemeOverridesLight')
                $script:CurrentWorkflow.Branding.Remove('ThemeOverridesDark')
