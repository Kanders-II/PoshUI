function Show-PoshUIDashboard {
    ...
    process {
        ...
        try {
            ...
            # Serialize to JSON instead of generating script
            Write-Verbose "Serializing UI definition to JSON"
            ...
            $jsonContent = Serialize-UIDefinition -Definition $script:CurrentWizard

            # Create SECURE temporary JSON file (using security framework)
            Write-Verbose "Creating secure temporary JSON file"
            $tempJsonPath = New-SecureTempFile -Content $jsonContent -Extension '.json'

            # Track temp file for cleanup
            if (-not $context.ContainsKey('TempFiles')) {
                $context['TempFiles'] = @()
            }
            $context.TempFiles += $tempJsonPath

            # Set signature enforcement mode based on parameter
            if ($RequireSignedScripts) {
                Write-Verbose "Script signature enforcement enabled via -RequireSignedScripts"
                $env:POSHUI_SIGNATURE_MODE = 'Enforce'
            } else {
                Write-Verbose "Script signature verification disabled"
                $env:POSHUI_SIGNATURE_MODE = 'Disabled'
            }

            try {
                # Execute the UI using SECURE invocation
                Write-Verbose "Invoking PoshUI.exe with secure execution"
                $result = Invoke-PoshUIExe -DefinitionPath $tempJsonPath -Wait
            } finally {
            ...
            $resultFilePath = [System.IO.Path]::ChangeExtension($tempJsonPath, '.result.json')
...
Set-Alias -Name 'Show-PoshDashboard' -Value 'Show-PoshUIDashboard'
